...
<script type="application/json" id="insumos-json">
    <?php echo $insumosJson; ?>
</script>
<script type="application/json" id="detalles-json-inicial">
    <?php echo $detallesJson; ?>
</script>

<meta name="csrf-token" content="<?php echo csrf_token(); ?>">

<script>
    const COMPRA_ID = <?php echo $compraId ?? 'null'; ?>;
    const RUTA_COMPRAS_STORE = "<?php echo route('compras.store'); ?>";
    const RUTA_COMPRAS_UPDATE = COMPRA_ID ? "<?php echo url('filament/resources/compras'); ?>/" + COMPRA_ID : null;
</script>

<!-- ✅ Carga tu JS personalizado -->
<script src="<?php echo asset('js/compra-form.js'); ?>"></script>

<form id="compra-form" class="space-y-6">
    <!-- Selección de bodega, proveedor y fecha -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
            <label for="bodega" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">Bodega de Destino</label>
            <select id="bodega" name="bodega_id" required
                ...
                <option value="{{ $bodega->id }}" @selected(($compra?->bodega_id ?? '') == $bodega->id)>
                    {{ $bodega->nombre }}
                </option>
                @endforeach
            </select>
